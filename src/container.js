...
import MovieSchema from './infrastructure/database/mongo/schemas/movie/movieSchema.js';
...
const configureContainer = async (config) => {
	container.register({
		...
		// schemas
		movieSchema: Awilix.asValue(MovieSchema),
	});
	...
	await container.loadModules(
		[
			...
			[
				'src/infrastructure/database/mongo/models/moviesModel.js',
				{
