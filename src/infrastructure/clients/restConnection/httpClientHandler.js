...
export default ({ httpConstants }) => ({
	...
	found: ({ data, status }) =>
		buildError(data, status, httpConstants.message.FOUND, BusinessException),
	...
	unused: ({ data, status }) =>
		buildError(data, status, httpConstants.message.UNUSED, BusinessException),
