...
const meta = (schema, key) => {
	const meta =
		schema.metas && schema.metas.find((meta) => meta[key] !== undefined);
...
const parseAsType = {
	...
	string: (schema) => {
		...
		if (find(schema.rules, { name: 'alphanum' })) {
			const strict =
				(schema.preferences && schema.preferences.convert) === false;
			swagger.pattern =
	...
	alternatives: (schema, existingComponents, newComponentsByRef) => {
		...
		Object.assign(newComponentsByRef, items.components || {});
	...
	object: (schema, existingComponents, newComponentsByRef) => {
		const requireds = [];
		const properties = {};
		const combinedComponents = Object.assign(
			{},
			existingComponents || {},
			newComponentsByRef || {}
		);
		...
		Object.keys(children).forEach((key) => {
			...
			Object.assign(newComponentsByRef, prop.components || {});
			Object.assign(combinedComponents, prop.components || {});

			properties[key] = prop.swagger;

			if (
				(child.flags && child.flags.presence) === 'required' ||
...
function parseDescription(schemaDescription, existingComponents) {
	if (
		(schemaDescription.flags && schemaDescription.flags.presence) ===
		'forbidden'
	)
		return false;
	...
	const components = {};
	const swagger = parseAsType[type](
		schemaDescription,
		existingComponents,
		components
	);
	...
	const description =
		schemaDescription.flags && schemaDescription.flags.description;
	...
	const defaultValue =
		schemaDescription.flags && schemaDescription.flags.default;
